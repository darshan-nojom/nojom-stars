...
public class SubmitContractJobActivity extends BaseActivity {

    private SubmitContractJobActivityVM submitJobActivityVM;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            contractId = getIntent().getIntExtra(Constants.PROJECT_BID_ID, 0);
            clientId = getIntent().getIntExtra("clientid", 0);
        }
        submitJobActivityVM = ViewModelProviders.of(this).get(SubmitContractJobActivityVM.class);
        submitJobActivityVM.init(this);
        initData();
        Utils.trackFirebaseEvent(this, "Submit_Gig_Contract_Screen");
    }
    ...
    private void initData() {
        ...
        binding.tvSubmitJob.setOnClickListener(v -> {
            ...
            if (fileList == null || fileList.size() == 0) {
                validationError(getString(R.string.please_select_one_file));
                return;
            }

            submitJobActivityVM.submitJob(contractId, getDescription());
        });
