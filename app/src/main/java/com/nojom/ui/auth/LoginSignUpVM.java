...
import static com.nojom.util.Constants.API_CHECK_UNIQUE;
import static com.nojom.util.Constants.API_CONTACT_AVAILABILITY;
...
import static com.nojom.util.Constants.API_RESET_PASSWORD_BY_PHONE;
import static com.nojom.util.Constants.API_RESET_PASSWORD_BY_TOKEN;
import static com.nojom.util.Constants.API_SEND_CODE;
import static com.nojom.util.Constants.API_SEND_OTP_PHONE;
...
public class LoginSignUpVM extends ViewModel implements APIRequest.APIRequestListener {
    ...
    public MutableLiveData<Boolean> isContactUnique = new MutableLiveData<>();
    ...
    public LoginSignUpVM(BaseActivity loginSignUpActivity) {
...
//        }
    ...
    public void forgotPassword(final String email, final boolean isResend, Button btn, ProgressBar progressBar, Dialog dialog, TextView textView) {
        if (!activity.isNetworkConnected())
            return;
    ...
    public void sendOTP(String email, String phone) {
        ...
        CommonRequest.SendOTP forgetPassword = new CommonRequest.SendOTP();
        ...
        APIRequest apiRequest = new APIRequest();
        apiRequest.makeAPIRequest(activity, API_SEND_CODE, forgetPassword.toString(), true, this);
    }

    public void checkContactExist(String email, String conatct) {
        if (!activity.isNetworkConnected())
            return;
        activity.disableEnableTouch(true);
        isShowProgressForget.postValue(true);

        CommonRequest.CheckContacts forgetPassword = new CommonRequest.CheckContacts();
        forgetPassword.setEmail(email);
        forgetPassword.setContact(conatct);

        APIRequest apiRequest = new APIRequest();
        apiRequest.makeAPIRequest(activity, API_CHECK_UNIQUE, forgetPassword.toString(), true, this);
    }

    public void sendOtpPhone(String conatct) {
        if (!activity.isNetworkConnected())
            return;
        activity.disableEnableTouch(true);
        isShowProgressForget.postValue(true);

        CommonRequest.SendOtpPhone forgetPassword = new CommonRequest.SendOtpPhone();
        forgetPassword.setPhone(conatct);

        APIRequest apiRequest = new APIRequest();
        apiRequest.makeAPIRequest(activity, API_SEND_OTP_PHONE, forgetPassword.toString(), true, this);
    }
    ...
    public void resetPasswordByToken(String jwt, String password) {
        if (!activity.isNetworkConnected())
            return;
        isShowProgressForget.postValue(true);
        activity.disableEnableTouch(true);

        CommonRequest.ResetPasswordByToken resetPassword = new CommonRequest.ResetPasswordByToken();
        resetPassword.setPassword(password);

        APIRequest apiRequest = new APIRequest();
        apiRequest.makeAPIRequest(jwt, activity, API_RESET_PASSWORD_BY_TOKEN, resetPassword.toString(), true, this);
    }
...
//        activity.disableEnableTouch(true);
//
//        CommonRequest.ResetPasswordByPhone resetPassword = new CommonRequest.ResetPasswordByPhone();
//        resetPassword.setPhone(phone);
//        resetPassword.setPassword(password);
//
//        APIRequest apiRequest = new APIRequest();
//        apiRequest.makeAPIRequest(activity, API_RESET_PASSWORD_BY_PHONE, resetPassword.toString(), true, this);
//    }

    public void checkContactExistOrNot(String contact) {
        if (!activity.isNetworkConnected())
            return;
        ...
        CommonRequest.CheckContact resetPassword = new CommonRequest.CheckContact();
        resetPassword.setContact(contact);

        APIRequest apiRequest = new APIRequest();
        apiRequest.makeAPIRequest(activity, API_CONTACT_AVAILABILITY, resetPassword.toString(), true, this);
    }
...
//                            String username = "agent_" + new Random().nextInt(10000);
    ...
    public void onResponseSuccess(String decryptedData, String urlEndPoint, String msg) {
        if (urlEndPoint.equalsIgnoreCase(API_FORGET_PASSWORD) || urlEndPoint.equalsIgnoreCase(API_CONTACT_AVAILABILITY)) {
            activity.toastMessage(msg);
            isShowProgressForget.postValue(false);
            sendCodeSuccess.postValue(true);
            /*if (!isResend) {
        ...
        } else if (urlEndPoint.equalsIgnoreCase(API_SEND_CODE)) {
            activity.toastMessage(msg);
            isShowProgressForget.postValue(false);
            sendCodeSuccess.postValue(true);
            /*if (!isResend) {
        ...
        } else if (urlEndPoint.equalsIgnoreCase(API_RESET_PASSWORD) || urlEndPoint.equalsIgnoreCase(API_RESET_PASSWORD_BY_TOKEN)) {
            activity.toastMessage(msg);
            isShowProgressForget.postValue(false);
//            progressDialog.dismiss();
            activity.redirectActivity(ResetPasswordDoneActivity.class);
            activity.finish();
        } else if (urlEndPoint.equalsIgnoreCase(API_CHECK_UNIQUE)) {
            //activity.toastMessage(msg);
            isContactUnique.postValue(true);
            isShowProgressForget.postValue(false);
        } else if (urlEndPoint.equalsIgnoreCase(API_SEND_OTP_PHONE)) {
            //activity.toastMessage(msg);
...
//            activity.start();
    ...
    public void onResponseError(Throwable t, String urlEndPoint, String message) {
        if (urlEndPoint.equalsIgnoreCase(API_FORGET_PASSWORD) || urlEndPoint.equalsIgnoreCase(API_CONTACT_AVAILABILITY)
                || urlEndPoint.equalsIgnoreCase(API_SEND_CODE)) {
...
//            progressBar.setVisibility(View.GONE);
            ...
            isContactUnique.postValue(false);
            isShowProgressForget.postValue(false);
        } else if (urlEndPoint.equalsIgnoreCase(API_SEND_OTP_PHONE)) {
            activity.toastMessage(message);
            isOtpSent.postValue(false);
            isShowProgressForget.postValue(false);
        }
        activity.disableEnableTouch(false);
    }
